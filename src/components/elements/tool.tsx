...
const getToolDisplayName = (type: string): string => {
  const toolNameMap: Record<string, string> = {
    'tool-readGoogleMeetRecording': 'Read Google Meet Recording',
  ...
  if (toolNameMap[type]) {
    return toolNameMap[type];
  }

  // Otherwise, format the tool name nicely
...
export const ToolOutput = ({
  ...
  return (
    <div className={cn('p-4', className)} {...props}>
      <div className="overflow-x-auto rounded-md bg-muted/50 text-xs [&_table]:w-full">
